    ...
    def __init__(self,data,d_,alpha = .5):
            ...
            for isweep in range(nSweeps):
                ...
                if(deltaE < 0 or random.random() < math.exp(-1.0*deltaE/T)):
                    if pid_swap_in != other_pid: # don't swap if we end up putting in a duplicate
                        ...
                        if points > best_points and salary <= self.MAX_SALARY:
                            best_points = points
                            best_salary = salary
        ...
        for pos in best_lineup:
            ...
            for pid in best_lineup[pos]:
                print ("%s," % self.d_id_to_name[pid]),
        print 'The actual points were ' + str(np.sum(np.vectorize(self.d_id_to_actual.get)(np.concatenate(best_lineup.values()))))